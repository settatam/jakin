...
<hr>
<?
        $k=mysql_query("SELECT post_id, page_title, image, post FROM posts WHERE page_id = 60 AND subcat = 4 order by post_id ASC LIMIT 3");	
		while($h=mysql_fetch_object($k)){
		?>
		<div class="large-4 columns" style="float: left;">
                <div class="border" ><img src="admin/uploaded_files/<?=$h->image?>.jpg" alt="">
					<h6><?=$h->page_title?></h6>
					<?=$h->post?>
				</div>
		</div>
...
<hr>
<?
        $k=mysql_query("SELECT post_id, page_title, image, post FROM posts WHERE page_id = 60 AND subcat = 1 order by post_id ASC LIMIT 3");	
		while($h=mysql_fetch_object($k)){
		?>
		<div class="large-4 columns" style="float: left;">
                <div class="border"><img src="admin/uploaded_files/<?=$h->image?>.jpg" alt="">
...
<hr>
	<?
        $k=mysql_query("SELECT post_id, page_title, image, post FROM posts WHERE page_id = 60 AND subcat = 2 order by post_id ASC LIMIT 3");	
		while($h=mysql_fetch_object($k)){
		?>
		<div class="large-4 columns" style="float: left;">
                <div class="border"><img src="admin/uploaded_files/<?=$h->image?>.jpg" alt="">
...
<h5 class="subheader">Project Staff</h5>
	<hr>
	<?
        $k=mysql_query("SELECT post_id, page_title, image, post FROM posts WHERE page_id = 60 AND subcat = 3 order by post_id ASC LIMIT 3");	
		while($h=mysql_fetch_object($k)){
		?>
		<div class="large-4 columns" style="float: left;">
                <div class="border"><img src="admin/uploaded_files/<?=$h->image?>.jpg" alt="">
